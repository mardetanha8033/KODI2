...
def MAIN(mode,url,text):
	...
	elif mode==371: results = TITLES(url)
...
def MENU(website=''):
	...
	if website=='':
		addMenuItem('folder',menu_name+'بحث في الموقع',website0a,379,'','','_REMEMBERRESULTS_')
...
def TITLES(url):
	...
	if html_blocks and html_blocks[0].count('href')>1:
		block = html_blocks[0]
		items = re.findall('href="(.*?)" title="(.*?)"',block,re.DOTALL)
		for link,title in items:
			link = website0a+'/'+link
		...
		allTitles = []
		html_blocks = re.findall('class="col-xs-12 title"(.*?)col-xs-12 ',html,re.DOTALL)
