...
def DELETE_CACHE():
	WHAT_IS_CACHE()
	yes = DIALOG_YESNO('هل تريد مسح جميع الكاش ؟','الكاش يسرع عمل البرنامج ومسحه يعيد سحب الصفحات من الأنترنيت عند الحاجة إليها والمسح يتم أوتوماتيكيا عند انتهاء عمر الصفحات والمسح لا يضر وممكن يحل بعض المشاكل','','','كلا','نعم')
	if yes:
		CLEAN_KODI_CACHE_FOLDER()
		DIALOG_OK('تم مسح كاش البرنامج بالكامل','إذا كانت عندك مشكلة في احد المواقع فجرب الموقع الآن ... وأذا المشكلة مستمرة فإذن ارسل المشكلة إلى المبرمج')
	return yes
