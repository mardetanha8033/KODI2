...
def TO_ADD():
	...
	XBMCGUI_DIALOG_OK('طريقة تحميل الملفات',message)
	return
...
def LIST_FILES():
	...
	for filename,mtime in files:
		#XBMCGUI_DIALOG_OK(filename,filename)
...
def CHANGE_FOLDER():
	downloadpath = GET_DOWNLOAD_FOLDER()
	change = XBMCGUI_DIALOG_YESNO(downloadpath,'هذا هو مكان تخزين ملفات الفيديو التي تحملها انت باستخدام هذا البرنامج . هل تريد تغيير المكان ؟','','','كلا','نعم')
	if change:
		newpath = XBMCGUI_DIALOG_BROWSESINGLE(3,'مكان تحميل ملفات الفيديو','local','',False,True,downloadpath)
		yes = XBMCGUI_DIALOG_YESNO(newpath,'هذا هو المكان الجديد لتخزين ملفات الفيديو التي تحملها انت باستخدام هذا البرنامج . هل تريد استخدامه بدلا من المكان القديم ؟','','','كلا','نعم')
		if yes:
			settings = xbmcaddon.Addon(id=addon_id)
			settings.setSetting('download.path',newpath)
			XBMCGUI_DIALOG_OK('رسالة من المبرمج','تم تغيير مكان تخزين الملفات المحملة')
	#if not change or not yes: XBMCGUI_DIALOG_OK('رسالة من المبرمج','تم الغاء العملية')
	return

def DOWNLOAD_VIDEO(url,videofiletype):
	XBMCGUI_DIALOG_NOTIFICATION('يرجى الانتظار','جاري فحص ملف التحميل',sound=False)
	LOG_THIS('NOTICE',LOGGING(script_name)+'   Preparing to download the video file   URL: [ '+url+' ]')
	#XBMCGUI_DIALOG_OK(url,videofiletype)
	if videofiletype=='':
	...
	if videofiletype not in ['.ts','.mkv','.mp4','.mp3','.flv','.m3u8','avi']:
		XBMCGUI_DIALOG_OK('تنزيل ملف الفيديو','الملف من نوع '+videofiletype+' والبرنامج حاليا غير جاهز لتحميل هذا النوع من الملفات')
		LOG_THIS('NOTICE',LOGGING(script_name)+'   Video type/extension is not supported   URL: [ '+url+' ]')
		return
	#XBMCGUI_DIALOG_OK('free space',str(freediskspace_MB))
	settings = xbmcaddon.Addon(id=addon_id)
	...
	#XBMCGUI_DIALOG_OK(downloadpath,filename)
	...
	#XBMCGUI_DIALOG_OK(url2,str(headers))
	#XBMCGUI_DIALOG_OK(xbmc.getInfoLabel('System.UsedSpace'),xbmc.getInfoLabel('System.TotalSpace'))
	#XBMCGUI_DIALOG_OK(xbmc.getInfoLabel('System.UsedSpacePercent'),xbmc.getInfoLabel('System.FreeSpacePercent'))
	MegaByte = 1024*1024
	...
	if filesize<=102400:
		...
		XBMCGUI_DIALOG_OK('رسالة من المبرمج','فشل في معرفة حجم ملف الفيديو ولهذا لا يمكن للبرنامج تحميل هذا الملف')
	...
	if freeafterdownload_MB<500:
		...
		XBMCGUI_DIALOG_OK('لا يوجد مساحة كافية للتحميل','الملف المطلوب تحميله حجمه '+str(filesize_MB)+' ميغابايت وجهازك فيه مساحة فارغة '+str(freediskspace_MB)+' ميغابايت وللمحافظة على عمل جهازك بدون مشاكل يجب ابقاء 500 ميغابايت فارغة دائما وهذا معناه جهازك لا توجد فيه مساحة كافية لتحميل ملف الفيديو المطلوب')
		file.close()
		return
	yes = XBMCGUI_DIALOG_YESNO('هل تريد تحميل الملف ؟','الملف المطلوب حجمه تقريبا '+str(filesize_MB)+' ميغابايت وجهازك فيه مساحة فارغة تقريبا '+str(freediskspace_MB)+' ميغابايت وهذا الملف قد يحتاج بعض الوقت للتحميل من الأنترنيت إلى جهازك . هل انت متأكد وتريد الاستمرار بتحميل ملف الفيديو ؟','','','كلا','نعم')
	if not yes:
		XBMCGUI_DIALOG_OK('','تم إلغاء عملية تحميل ملف الفيديو')
		file.close()
	...
	if not Finished:
		...
		XBMCGUI_DIALOG_OK('','تم تحميل ملف الفيديو بنجاح')
		return
